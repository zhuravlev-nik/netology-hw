# Домашнее задание к занятию "`Управляющие конструкции в коде Terraform`" - `Журавлев Николай`
...
1. Неверно закрыта фигурная скобка в переменной подставляемой в ansible_host
2. Пробел в ключе "platform_id "
Исправленный шаблон:
```
[webservers]
%{~ for i in webservers ~}
...
```hcl
[for i in range(1, 100) : "rc${i}"],
[for i in range(1, 100) : "rc${i}" if !contains(["0", "7", "8", "9"], substr(tostring(i), length(tostring(i)) - 1, 1)) || i == 19]
```